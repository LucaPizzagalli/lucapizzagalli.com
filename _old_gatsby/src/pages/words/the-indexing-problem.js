...
// slug: "/words/the-indexing-problem"
// title: "The Indexing Problem"
// date: "2026-05-11"
// order: 1
// ---

function TheIndexingProblemPage({ loop }) {
  return (
    <Layout loop={loop} location="/words/the-indexing-problem">
      ...
      <p>
        This universe possesses a symmetry such that it is invariant by rotations of <InlineMath math="\pi/3" /> degrees, with only one exception breaking the symmetry: One of the lamps - let's call it the North lamp - contains a timer. There is a countdown, and when it reaches <InlineMath math="t=0" />, this and only this lamp will turn on.
      </p>
      <p>
        ...
        The North lamp provides and absolute point of reference; we may assume the entities encode the universe in polar coordinates, with North lamp fixed at <InlineMath math="\theta = 0" />.
      </p>

      <figure>
        <img
          src="./the-indexing-problem/universe.svg"
          alt=""
          style={{ width: "80%", "max-width": "30rem", filter: "var(--green-filter) var(--noise-filter) var(--noise2-filter)" }} />
      ...
      <p>
        Here lies the central question. Let's label each entity on the planet: A is at <InlineMath math="\theta = 0" /> under the North lamp, and continuing clockwise we have B at <InlineMath math="\theta = \pi/3" />, then C, D, E, and F.
        For <InlineMath math="t<0" />, the identical animals could perfectly predict the evolution of the universe. They knew that at <InlineMath math="t>0" />, the entity at A would be in light, and the entity at D would be in darkness. They could also perfectly predict their own identical sensory experiences for <InlineMath math="t<0" />.
      </p>
      <p>
        However, there is one crucial piece of information they could not predict: their own identical sensory experiences for <InlineMath math="t>0" />, equivalently <em>which of the individuals they actually are</em>.
      ...
      <p>
        There is no "I". The universe locally processes information. A brain contains memories of its own past, not the past of other brains. Being a conscious brain in an instant <InlineMath math="t" /> means having access only to the memory of that brain, which creates the <em>illusion</em> of a continuous identity across time.
      </p>
      <p>
        Before <InlineMath math="t=0" />, there were simply 6 identical local processors computing the exact same data. After <InlineMath math="t=0" />, the new asymmetrical inputs caused the 6 local processors to compute different data. There is no metaphysical "I" that travelled from the past to the future.
      </p>
    </Layout>
  );
}

export default TheIndexingProblemPage
